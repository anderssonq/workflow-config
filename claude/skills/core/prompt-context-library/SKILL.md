...
description: Saves and retrieves reusable prompts and project context as markdown. Load when asked to save, load or list a context, or when about to retype instructions given before.
---
...
- The context is a rule the project must always obey → that is a skill, see
  `skill-authoring`. A saved context is convenience; a skill is law.
- The context is something an agent learned and should keep → `memory-system`.
