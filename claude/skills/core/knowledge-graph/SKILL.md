...
description: Maps a project into a queryable knowledge graph instead of grepping. Load when asked to map, graph or explain a codebase, or to trace how one part connects to another.
---
...
- A question one grep answers. Building a graph to find a function is expensive theatre.
- Reviewing a change for defects → `code-reviewer`.
- Documenting the architecture for humans → `playbooks/documentation-system.md`. A graph is
  a tool for answering questions, not a document of record.
