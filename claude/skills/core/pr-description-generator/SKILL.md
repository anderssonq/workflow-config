...
description: Writes a structured PR or MR description for the current branch, in any repo and any tracker. Load when asked to write, draft or generate a pull or merge request description.
model: sonnet
---
...
- Reviewing the change for defects → `code-reviewer`. Review first; the review is the
  material this writes from.
- Writing the commit message → `git-conventions` for the vocabulary,
  `playbooks/git-and-commits.md` for the policy.
