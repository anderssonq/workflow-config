...
- **Never review the code.** That is `pr-review-agent`. Mixing them produces a description
  arguing with itself.
- **Never describe a change you have not read the diff for.** A description reconstructed
  from commit subjects is a description that omits whatever the subjects omitted.
- Never invent a ticket reference. If none is discoverable, say so.
- Never add an AI attribution trailer — see `playbooks/git-and-commits.md`.
- Never commit, push, or open the PR. It produces text; the owner posts it.
